...
#
mean1, cov1 = np.array([1, 1]), np.array([(0.5, .0), (.0, .5)])
mean2, cov2 = np.array([-1.5,-1.5]), np.array([[1.,0],[0,1.]])
...
def U(q):
    ...
    for i in range(q.shape[0]):
        res[i] = - np.log(1 / 4 * np.exp( - np.dot(np.dot(q[i] - mean1,inv_cov1), q[i] - mean1)/2 )  + 3 / 4 * np.exp( - np.dot(np.dot(q[i] - mean2,inv_cov2), q[i] - mean2)/2 )+1e-5)
    return res
...
grad_U = lambda q:-1/(U(q.reshape(D,)))*((1/4)*np.exp(- np.dot(np.dot(q.reshape(D,) - mean1,inv_cov1), q.reshape(D,) - mean1)/2 )*np.dot((-q.reshape(D,)+mean1),inv_cov1) + (3/4)*np.exp(- np.dot(np.dot(q.reshape(D,) - mean2,inv_cov2), q.reshape(D,) - mean2)/2 )*np.dot((-q+mean2),inv_cov2))
...
Z = (1/4)*bivariate_normal(X, Y, mean1,cov1) + (3/4)*bivariate_normal(X, Y, mean2,cov2)
...
samples = int(q_hist.shape[0]/2)

ax1= plt.contour(X,Y,Z)
plt.scatter(q_hist[samples:, 0], q_hist[samples:, 1])
plt.show()
